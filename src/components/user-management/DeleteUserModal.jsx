...
/**
 * DeleteUserModal - Confirmation modal when deleting a user account.
 */
export default function DeleteUserModal({ isOpen, onClose, onConfirm, userName, isDeleting }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4" dir="rtl">
      <div className="bg-base-100 border border-base-300 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4 text-center animate-fadeIn">
        ...
        <p className="font-2 text-sm text-base-content/70">
          هل أنت تأكد من إرادتك لحذف حساب <span className="font-bold text-base-content">"{userName}"</span>؟ لا يمكن التراجع عن هذا الإجراء وسيتم إلغاء صلاحيات الحساب.
        </p>
