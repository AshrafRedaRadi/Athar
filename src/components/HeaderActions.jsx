...
export default function HeaderActions({ className = "", buttonClassName = "" }) {
  ...
  const baseBtnClass =
    buttonClassName ||
    "btn btn-ghost btn-sm btn-circle text-base-content/70 hover:text-cyan-700 transition-colors";

  return (
    <div className={`flex items-center gap-1 sm:gap-2 ${className}`}>
      {/* Sun / Moon Theme Toggle */}
      <label className={`swap swap-rotate ${baseBtnClass}`} title="تغيير المظهر">
