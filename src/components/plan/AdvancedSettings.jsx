...
const AdvancedSettings = () => {
  ...
  return (
    <div className="bg-base-100 p-6 rounded-3xl shadow-sm border border-base-300 w-full max-w-sm space-y-5 font-2" dir="rtl">
      ...
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setStrictOrder((prev) => !prev)}>
        <input
          type="checkbox"
          checked={strictOrder}
          onChange={(e) => setStrictOrder(e.target.checked)}
          onClick={(e) => e.stopPropagation()}
          className="toggle bg-base-300 border-base-300 checked:bg-cyan-700 checked:border-cyan-700 checked:[--tglbg:white] transition-colors duration-200 cursor-pointer"
        />
      ...
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setNotifications((prev) => !prev)}>
        <input
          type="checkbox"
      ...
      {/* Modern Glassmorphism Time Input Card */}
      <div className="space-y-2">
        ...
        >
          ...
          {/* Custom Formatted Time Input Display */}
