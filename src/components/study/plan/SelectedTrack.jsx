...
const SelectedTrack = (props) => {
  // استقبال قائمة المسارات من الـ props أو استخدام مصفوفة افتراضية تشبه الـ API
  ...
  // القيمة المحددة حالياً (المختار)
  const [selectedId, setSelectedId] = useState(props.selectedId || tracks[0]?.id);

  const handleSelect = (id) => {
    setSelectedId(id);
    // إرسال خيار المستخدم إلى الـ Component الأب إن وجد callback
    if (props.onSelect) {
      props.onSelect(id);
    }
  };
  ...
  return (
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 w-full max-w-sm space-y-4 font-sans" dir="rtl">
      ...
      <div className="flex items-center justify-start gap-2 text-gray-900 font-bold text-lg mb-2">
        ...
        {/* أيقونة القائمة/الدفتري */}
        <svg className="w-6 h-6 text-[#9A6B29]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
        </svg>
      </div>

      {/* عرض مصفوفة المسارات القادمة من الـ API */}
      <div className="space-y-3">
        {tracks.map((track) => {
          ...
          return (
            ...
            >
              {/* زر الاختيار الدائري (Radio Indicator) */}
